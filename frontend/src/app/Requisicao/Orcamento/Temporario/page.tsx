...
export default function Page() {
  ...
  const [taxaAdministrativa, setTaxaAdministrativa] = useState<number>(1.15)
  ...
  const [state, formAction, isPending] = useActionState(RegistrarOcamento, null)
  ...
  useEffect(() => {
    if (usuario?.CodigoCliente !== codigoCliente[0]) {
      if (usuario?.CodigosCliente?.length > 0) {
        setCodigoCliente([usuario.CodigosCliente])
      }
    }
  }, [usuario])

  useEffect(() => {
    const pegarContratos = async () => {
      try {
        if (codigoCliente.length > 0 && codigoCliente[0] !== null) {
          const data = await getContratosAtivos(codigoCliente[0] as unknown as number[])
          setContratos(data)
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    // Filtrar os contratos com o codigo de faturamento do tipo 3 "Temporarios"
    const contratosTemporarios = contratos.filter((c: any) => c.TipoFaturamento === 1)

    const maiorTaxaContrato = contratosTemporarios.reduce(
      (maxContrato: any, contrato: any) =>
        contrato.TaxaFatur > (maxContrato?.TaxaFatur ?? -Infinity) ? contrato : maxContrato,
      null
    )
    if (maiorTaxaContrato !== null && maiorTaxaContrato > 1.15) {
      //&& maiorTaxaContrato > 1.15
      maiorTaxaContrato.TaxaFatur
      setTaxaAdministrativa(maiorTaxaContrato.TaxaFatur)
    }

    console.log(maiorTaxaContrato)
  }, [contratos])

  useEffect(() => {
    setEncargosSelecionados(encargos['Temporário'])
  ...
  useEffect(() => {
    ...
    setValorAdministrativa(subtotalNotaFiscal * taxaAdministrativa)
        ...
        <div className="mb-5 flex items-center gap-3">
          <div className="h-full w-full rounded-md border border-gray-300 bg-white">
            <h1 className="justify-center p-2 text-center text-2xl font-bold text-gray-800">
              Simulação de Orçamento Temporário
            </h1>
            ...
            <div className="flex h-full w-full flex-col p-5">
                ...
                <div className="relative grid grid-cols-1 items-center justify-start">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      if (selectedOption) {
                        const parsedISS = parseFloat(selectedOption.ISS.replace(',', '.')).toFixed(
                          2
                        )
                      ...
                      <div key={grupo} className="mb-5 grid w-full grid-flow-row grid-cols-4">
                        ...
                        {Array.isArray(lista) ? (
                          <>
                            {lista.map((encargo, idx) => (
                              <Fragment key={idx}>
                                ...
                                <input
                                  ...
                                  name={`pencentual-encargos-${encargo.nome.toLowerCase().replace(/\s+/g, '-')}`}
                                  id={`pencentual-encargos-${encargo.nome.toLowerCase().replace(/\s+/g, '-')}`}
                                  type="text"
                                  defaultValue={
                            ...
                            <div className="border border-purple-300 bg-purple-100">
                              <input
                                className="h-full w-full p-2 text-center text-lg font-bold text-gray-800"
                                name={`percentual-total-${grupo.toLowerCase().replace(/\s+/g, '-')}`}
                                id={`percentual-total-${grupo.toLowerCase().replace(/\s+/g, '-')}`}
                                type="text"
                                defaultValue={
                                  lista
              ...
              <div className="grid h-full w-full grid-cols-8">
                      ...
                      <h2 className="rounded-tl-md border border-purple-300 bg-purple-100 p-2 text-lg font-bold text-gray-800">
                        ...
                        <div className="group relative inline-block">
                          ...
                          <p className="text-info absolute bottom-full mb-2 hidden max-h-[110px] min-w-[500px] rounded-lg bg-gray-800 px-3 py-1 text-sm text-white shadow-lg group-hover:block">
                            ...
                            periodicidades mensal e diária. Benefícios mensais refletem o custo
                            total para o período de um mês. Benefícios diários são calculados por
                            dia de uso, multiplicados por 22 dias úteis, conforme as quantidades e
                            valores unitários informados.
                          </p>
                        </div>
                      </h2>
                    </div>
                    ...
                    >
                      ...
                      <div className="border border-gray-300 bg-inherit p-2 text-gray-800">
                        <input
                          ...
                          value={
                            checkBoxStatus[key] ? beneficiosCalculos[key]?.quantidade || '' : ''
                          }
                          onChange={(e) => {
                      ...
                      <div className="border border-gray-300 bg-inherit p-2 text-gray-800">
                        <NumericFormat
                          ...
                          allowNegative={false}
                          value={
                            checkBoxStatus[key] ? beneficiosCalculos[key]?.fornecido || '' : ''
                          }
                          readOnly
                        />
                      </div>
                ...
                <div className="col-span-8 my-5 grid h-full w-full items-center justify-center gap-1 rounded-md border border-gray-200 bg-gray-50 p-2 text-sm text-gray-700 italic">
                  ...
                  <p>
                    Os custos com Adicional Noturno, Insalubridade, Periculosidade, Horas Extras,
                    DSR, Bônus e Prêmios são considerados como remuneração, tendo incidências de
                    todos os encargos sociais, previdenciários, tributários e Taxa administrativa. *
                    De acordo com o disposto no artigo 3º, inciso XX (item 17.05) , da Lei
                    Complementar 116, as notas fiscais - faturas da prestação de serviços serão
                    emitidas ao tomador de serviços (cliente) e o ISS deverá ser retido em favor do
                    município da prestação de serviços. O valor da exame médico admissional será
